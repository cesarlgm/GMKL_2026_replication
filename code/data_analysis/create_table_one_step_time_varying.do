...
cap program drop create_one_steptime
program define create_one_steptime
	syntax, [NOsen]
	clear 
	...
	forvalues j=1/3 {
		estimates use "results/regressions/one_step_`j'_clean"
		eststo one_step_`j'
		
		estimates use "results/regressions/one_step_nofe_`j'_clean"
		eststo one_step_nofe_`j'
	}
	...
	*Outputting tex table of university level stats
	local table_name	"results/tables/table_one_step_time_varying.tex"
	local table_key 	"tab:one_step_time_varying"
	local table_title 	"Effect of time-varying variables, one-step estimates"
	local coltitles		""
	local n_cols		6
	...
	local models		one_step_1 one_step_2 one_step_3 one_step_nofe_1 one_step_nofe_2 one_step_nofe_3
	local exhead
	...
	local coeflabels2 	1.faculty_rank_f "$texspace Lecturer" 2.faculty_rank_f "$texspace Instructor" ///
						...
						time_current_job_f	 "Time in current job"		
	...
	leanesttab `models' using `table_name', keep(`keep2')  append nobase noomit ///
		indicate("\midrule Individual FE=*.panelid" "Year FE=*refyr", label("$ \checkmark $" "")) format(4) nostar coeflabel( `coeflabels2') noobs stats(N n_movers r2, label("\midrule Observations" "Number of movers" "$ R^2 $ ") ///
