...
*	Description: 	Master orchestrator for the full data construction pipeline.
*					Processes 13 SDR biennial waves into a unified work-history
*					dataset, cleans job spells for institution-switchers and
*					non-switchers, and constructs individual- and
*					institution-level databases. Implements a two-pass AKM
*					estimation strategy: a first (temporary) pass identifies
*					the connected set and trims the sample to the correct 653
*					institutions; a second (final) pass produces the published
*					wage fixed effects. Also produces auxiliary estimates
*					(one-step, tenured-only, field-residualized, grouped, and
*					job-satisfaction) and applies the Andrews et al. (2008)
*					leave-out variance correction via R.
...
*Loading basic regression-related programs used across the code  
...
*Adding IPEDs codes and cretting a file with a more restricted sample    
